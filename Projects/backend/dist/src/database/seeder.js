...
exports.randomNeighborhood = randomNeighborhood;
exports.randomDetails = randomDetails;
...
const AppDataSource = new typeorm_1.DataSource({
    ...
    password: 'postgres',
    database: 'postgres',
    entities: [projects_entity_1.Project, properties_entity_1.Property, lands_entity_1.Land],
    synchronize: false,
});
...
async function seed() {
    ...
    const projectRepo = AppDataSource.getRepository(projects_entity_1.Project);
    const propertyRepo = AppDataSource.getRepository(properties_entity_1.Property);
    const landRepo = AppDataSource.getRepository(lands_entity_1.Land);
    const projects = [];
    for (let i = 0; i < 400; i++) {
        projects.push(projectRepo.create({
            name: `Project ${i + 1}`,
    ...
    const properties = [];
    for (let i = 0; i < 400; i++) {
        properties.push(propertyRepo.create({
            name: `Property ${i + 1}`,
